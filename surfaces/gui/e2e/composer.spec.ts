...
test("composer: send-gating, + attach menu, Mode menu", async ({ page }) => {
  ...
  // "+" attach menu: Image (inline attach) + Files or folder (routes through RAG
  // ingestion — U3 consolidated the old separate PDF/"Other files" buttons into this
  // one entry, with its own file-vs-folder sub-flyout).
  await page.getByRole("button", { name: "Attach" }).click();
  await expect(page.getByRole("button", { name: "Photo or image" })).toBeVisible();
  await expect(page.getByTestId("attach-files-row")).toBeVisible();
