...
/** "Register a worker" (WorkerRegistrationSection): a fully custom worker with its own
 * system prompt, alongside the simpler general-worker + skills path on the Team page's
 * worker box. Default ON pending a decision after testing;
 * `ocw.flag.worker_registration` = "0" hides it. */
export const showWorkerRegistration = () => flag("ocw.flag.worker_registration", true);
