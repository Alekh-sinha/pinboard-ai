...
function ComposerPickerCard({
  ...
  // Worker pool (the ordered fallback a staffed worker switches to on a quota/access
  // failure — get_default_worker_model_pool/set_default_worker_model_pool,
  // already built and already used by create_team()'s fallback): a "use in worker
  // pool" mark against this same curated list, right where it's reviewed, rather than
  // a separate list-management surface.
  const [pool, setPool] = useState<string[]>([]);
  useEffect(() => {
    getWorkerModelPool().then(setPool).catch(() => {});
  }, []);
  const togglePool = (id: string) => {
    const next = pool.includes(id) ? pool.filter((m) => m !== id) : [...pool, id];
    setPool(next);
    void setWorkerModelPool(next);
  };
  return (
        ...
        {settings.models.map((id) => {
          ...
          return (
            <div className="mlist-row" key={id}>
              ...
              <label className="flex items-center gap-1.5 text-[11.5px] text-muted mr-2 shrink-0 cursor-pointer">
                <input
                  type="checkbox"
                  data-testid={`worker-pool-${id}`}
                  checked={pool.includes(id)}
                  onChange={() => togglePool(id)}
                />
                {t("model_manage.worker_pool")}
              </label>
              {isDefault ? (
