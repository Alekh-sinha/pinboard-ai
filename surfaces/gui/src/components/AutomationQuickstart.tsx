...
export function AutomationQuickstart({
  ...
  return (
    <div className="mb-4">
      ...
      <div className="grid grid-cols-3 auto-rows-fr gap-3.5">
        {TEMPLATES.map((tpl) => {
          const { color, rotate } = noteVariant(tpl.key);
          const isPicked = pickedKey === tpl.key;
          return (
          <button
            key={tpl.key}
            data-testid={`qs-template-${tpl.key}`}
            className={
              "note note-tmpl flex flex-col gap-1.5 " + color + (isPicked ? " picked" : "")
            }
            style={{ transform: `rotate(${isPicked ? 0 : rotate}deg)` }}
            onClick={() => pick(tpl)}
          >
      ...
      {picked && (
        <div
          ref={cfgRef}
          className={"note note-configure mt-3 " + noteVariant(picked.key).color}
        ...
        >
          ...
          {allConnected && (
            <div className={picked.conns.length ? "note-inset rounded-xl px-4 py-3.5 mt-3" : ""} data-testid="ob-recipe">
              {picked.needsRepo && (
                <>
