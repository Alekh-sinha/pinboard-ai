...
import { GeneralTeamToggle } from "./GeneralTeamToggle";
...
export function ConnectorsList({
  ...
  // general-lead/general-worker's own connector defaults (C3) — reuses the existing
  // per-persona connections mechanism (§5 of docs/connector-architecture.md) against
  // these two fixed ids. No opinion yet = on by default (both declare
  // `connectors: all` with no `recommends:`), so absence from default_connections
  // means "checked".
  const [leadConn, setLeadConn] = useState<Record<string, boolean>>({});
  const [workerConn, setWorkerConn] = useState<Record<string, boolean>>({});
  useEffect(() => {
    getPersonaDetail("general-lead")
      .then((d) =>
        setLeadConn(Object.fromEntries(d.default_connections.map((c) => [c.connector, c.enabled]))),
      )
      .catch(() => {});
    getPersonaDetail("general-worker")
      .then((d) =>
        setWorkerConn(Object.fromEntries(d.default_connections.map((c) => [c.connector, c.enabled]))),
      )
      .catch(() => {});
  }, []);
  const toggleFor = (
    persona: "general-lead" | "general-worker",
    setter: React.Dispatch<React.SetStateAction<Record<string, boolean>>>,
  ) => (connector: string, next: boolean) => {
    setter((cur) => ({ ...cur, [connector]: next }));
    void setPersonaConnection(persona, connector, next);
  };
  const toggleLead = toggleFor("general-lead", setLeadConn);
  const toggleWorker = toggleFor("general-worker", setWorkerConn);
          ...
          <div className={GRP}>
            {connected.map((c) => (
              ...
              >
                ...
                <GeneralTeamToggle
                  leadOn={leadConn[c.name] ?? true}
                  workerOn={workerConn[c.name] ?? true}
                  onToggleLead={(next) => toggleLead(c.name, next)}
                  onToggleWorker={(next) => toggleWorker(c.name, next)}
                />
