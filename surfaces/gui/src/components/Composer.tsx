...
import { chooseFile, chooseFolder } from "../tauri";
...
interface Props {
  ...
  onOpenMemory?: () => void;
  // "+" menu redesign (Team Setup plan, U2): Persona is a REAL inline picker (same
  // list + onPickCoworker callback SessionSetupRow uses for a fresh/draft session),
  // not a navigation-only link — pass `personas`/`agent`/`onPickCoworker` only when
  // picking a coworker is valid right now (pre-first-message); the row still renders
  // as a plain "Manage coworkers" link otherwise. Connectors is a glance (current
  // connectors/MCP servers) + "Manage" link into the Connectors page. Skills is a
  // plain shortcut to the existing global Skills tab. Model is NOT here — the
  // existing right-side model control is extended instead (see B4).
  personas?: Persona[] | null;
  agent?: string;
  onPickCoworker?: (id: string) => void;
  onManageCoworkers?: () => void;
  // Team page (C2): a whole surface (two boxes — lead picker, general-worker skill
  // panel), not a composer flyout.
  onOpenTeam?: () => void;
  onOpenConnectors?: () => void;
  onOpenSkills?: () => void;
  // Model-manage gear next to the model picker: opens Settings ▸ Models directly
  // (the existing add/remove-model + worker-pool-checkbox surface) rather than a
  // separate page.
  onOpenModels?: () => void;
...
export function Composer(props: Props) {
  ...
  // "+" menu redesign (U2): three sibling flyouts off the main panel — Persona (an
  // inline picker), Connectors (a glance), Files/Folder (choose file vs. folder).
  const [personaMenuOpen, setPersonaMenuOpen] = useState(false);
  // Persona flyout's own Solo/Team split (C1): "Solo" nests a third-level picker;
  // "Team" navigates straight to the Team page (a whole surface, not a flyout).
  const [soloMenuOpen, setSoloMenuOpen] = useState(false);
  const [connectorsMenuOpen, setConnectorsMenuOpen] = useState(false);
  const [filesMenuOpen, setFilesMenuOpen] = useState(false);
  const [connectorsGlance, setConnectorsGlance] = useState<{
    connectors: Connector[];
    mcp: McpServer[];
  } | null>(null);
  // Team Setup (U5): folder/file ingestion status — a deterministic backend job
  // (server/manager.py's start_rag_ingest), never a model tool call, so this is a
  // plain progress poll, not something the agent's turn stream reports.
  const [ragStatus, setRagStatus] = useState<string | null>(null);
  // Checked = today's automatic behavior (OCR only for pages that need it).
  // Unchecked = a hard, complete opt-out for this ingest job — every page parses
  // without OCR regardless of how it looks, not just a soft preference.
  ...
  // Team Setup (U5/U3): a file OR folder attach calls the deterministic ingestion
  // endpoint directly — never a model tool call — so it happens whether or not the
  // agent remembers to ask for it. See server/manager.py's start_rag_ingest, which
  // already accepts a single file path as well as a directory.
  const ingestIntoKnowledgeBase = async (picked: string | null) => {
    setAttachMenuOpen(false);
    setFilesMenuOpen(false);
    if (!picked) return;
    const collection = baseName(props.workspace || picked) || "default";
    const started = await startRagIngest(picked, collection, props.workspace, ocrEnabled);
    if (!started.ok || !started.job_id) {
      setRagStatus(started.error || t("composer.rag_ingest_failed"));
      return;
    }
    const jobId = started.job_id;
    setRagStatus(t("composer.rag_ingest_running", { done: 0, total: started.total ?? 0 }));
    const poll = async () => {
      const s = await getRagIngestStatus(jobId);
      if (s.status === "done") {
        setRagStatus(t("composer.rag_ingest_done", { count: s.done ?? 0, collection }));
        setTimeout(() => setRagStatus(null), 8000);
        return;
      }
      if (s.status === "error") {
        setRagStatus(s.error || t("composer.rag_ingest_failed"));
        return;
      }
      setRagStatus(
        t("composer.rag_ingest_running", { done: s.done ?? 0, total: s.total ?? 0 }) +
          (s.current ? ` — ${s.current}` : ""),
      );
  ...
  const addFolderToKnowledgeBase = () => void chooseFolder().then(ingestIntoKnowledgeBase);
  const addFileToKnowledgeBase = () => void chooseFile().then(ingestIntoKnowledgeBase);

  // "+" menu Connectors glance (U2): fetched once per menu open, same fetch-on-open
  // idiom as the "/" skill popup above — no standing poll for a menu nobody's looking at.
  useEffect(() => {
    if (!connectorsMenuOpen || connectorsGlance) return;
    Promise.all([getConnectors(), getMcpServers()])
      .then(([connectors, mcp]) => setConnectorsGlance({ connectors, mcp }))
      .catch(() => setConnectorsGlance({ connectors: [], mcp: [] }));
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [connectorsMenuOpen]);
  ...
  return (
    <div className="composer-wrap px-6 pb-5 pt-4">
      ...
      {/* Knowledge-base ingestion progress (U5) — a plain status poll, not a turn event. */}
      {ragStatus && (
        <div
          data-testid="rag-ingest-notice"
          className="max-w-3xl mx-auto mb-1.5 flex items-center gap-2 rounded-lg border border-line bg-paper px-3 py-1.5 text-[13px] text-muted"
        >
          <span className="flex-1">{ragStatus}</span>
          <button
        ...
        <div className="px-2.5 pb-2.5 pt-1 flex items-center gap-1.5">
            ...
            {attachMenuOpen && (
              <>
                ...
                <div className="absolute z-40 bottom-full mb-1 left-0 min-w-[200px] rounded-xl border border-line bg-panel shadow-2xl py-1.5">
                  ...
                  {/* Files or folder (U3): a single entry routing to the deterministic RAG
                      ingestion pipeline — one file via startRagIngest's file-path branch, a
                      folder via its directory walk. Replaces the old separate "PDF" / "Other
                      files" inline-attach items and the standalone "Add folder to knowledge
                      base" button, which all did overlapping things under different names. */}
                  <div className="relative">
                    <button
                      className="w-full flex items-center gap-2.5 px-3 py-1.5 text-[13px] text-left hover:bg-paper"
                      data-testid="attach-files-row"
                      onClick={(e) => {
                        e.stopPropagation();
                        setFilesMenuOpen((v) => !v);
                      }}
                    >
                      <Icon name="folderPlus" size={15} className="shrink-0 text-muted" />
                      {t("composer.attach_files_or_folder")}
                      <Icon name="chevronRight" size={12} className="ml-auto text-faint" />
                    </button>
                    {filesMenuOpen && (
                      <div
                        className="absolute z-40 left-full top-0 ml-1 min-w-[220px] rounded-xl border border-line bg-panel shadow-2xl py-1.5"
                        onClick={(e) => e.stopPropagation()}
                      >
                        <button
                          className="w-full flex items-center gap-2.5 px-3 py-1.5 text-[13px] text-left hover:bg-paper"
                          data-testid="attach-file-kb"
                          onClick={() => void addFileToKnowledgeBase()}
                        >
                          {t("composer.attach_choose_file")}
                        </button>
                        <button
                          className="w-full flex items-center gap-2.5 px-3 py-1.5 text-[13px] text-left hover:bg-paper"
                          data-testid="attach-folder-kb"
                          onClick={() => void addFolderToKnowledgeBase()}
                        >
                          {t("composer.attach_choose_folder")}
                        </button>
                        <div className="my-1 border-t border-line" />
                        <label className="w-full flex items-center gap-2.5 px-3 py-1.5 text-[13px] hover:bg-paper cursor-pointer">
                          <input
                            type="checkbox"
                            data-testid="attach-folder-ocr-toggle"
                            checked={ocrEnabled}
                            onChange={(e) => setOcrEnabled(e.target.checked)}
                          />
                          <span className="flex-1">{t("composer.attach_folder_ocr")}</span>
                        </label>
                      </div>
                    )}
                  </div>
                  ...
                  {/* Persona: Solo vs. Team split. "Solo" is the real inline picker (only
                      valid pre-first-message — App.tsx passes personas/agent/onPickCoworker
                      only then, same gating as SessionSetupRow), filtered to non-team
                      personas. "Team" opens the dedicated Team page (two boxes: lead
                      picker, general-worker skill panel) rather than a composer flyout —
                      staffing/team concerns deserve more room than a dropdown. */}
                  <div className="relative">
                    <button
                      className="w-full flex items-center gap-2.5 px-3 py-1.5 text-[13px] text-left hover:bg-paper"
                      data-testid="attach-persona-row"
                      onClick={(e) => {
                        e.stopPropagation();
                        if (props.personas && props.onPickCoworker) {
                          setPersonaMenuOpen((v) => !v);
                        } else {
                          setAttachMenuOpen(false);
                          props.onManageCoworkers?.();
                        }
                      }}
                    >
                      <Icon name="sparkle" size={15} className="shrink-0 text-muted" />
                      {t("composer.attach_personas_persona")}
                      {props.personas && props.onPickCoworker && (
                        <Icon name="chevronRight" size={12} className="ml-auto text-faint" />
                      )}
                    </button>
                    {personaMenuOpen && props.personas && props.onPickCoworker && (
                      <div
                        className="absolute z-40 left-full top-0 ml-1 w-[220px] rounded-xl border border-line bg-panel shadow-2xl py-1.5"
                        onClick={(e) => e.stopPropagation()}
                      >
                        <div className="relative">
                          <button
                            className="w-full flex items-center gap-2.5 px-3 py-1.5 text-[13px] text-left hover:bg-paper"
                            data-testid="attach-solo-row"
                            onClick={() => setSoloMenuOpen((v) => !v)}
                          >
                            {t("composer.attach_solo")}
                            <Icon name="chevronRight" size={12} className="ml-auto text-faint" />
                          </button>
                          {soloMenuOpen && (
                            <div
                              className="absolute z-40 left-full top-0 ml-1 w-[260px] max-h-[320px] overflow-y-auto rounded-xl border border-line bg-panel shadow-2xl py-1.5"
                              onClick={(e) => e.stopPropagation()}
                            >
                              {props.personas
                                .filter((p) => p.enabled && !p.team)
                                .map((p) => (
                                  <button
                                    key={p.id}
                                    data-testid={`attach-persona-${p.id}`}
                                    className={
                                      "w-full text-left px-3 py-1.5 rounded-lg hover:bg-paper " +
                              ...
                              {props.onManageCoworkers && (
                                <>
                                  <div className="my-1 border-t border-line" />
                                  <button
                                    className="w-full text-left px-3 py-1.5 rounded-lg hover:bg-paper text-[12px] text-accent"
                                    data-testid="attach-manage-coworkers"
                                    onClick={() => {
                                      setAttachMenuOpen(false);
                                      setPersonaMenuOpen(false);
                                      setSoloMenuOpen(false);
                  ...
                  {/* Connectors (U2): a glance at what's currently reachable + a Manage
                      link into the Connectors page, where the role-scope (lead / general
                      worker / both) toggle actually lives. */}
                  {props.onOpenConnectors && (
                    <div className="relative">
                      <button
                        className="w-full flex items-center gap-2.5 px-3 py-1.5 text-[13px] text-left hover:bg-paper"
                        data-testid="attach-connectors-row"
                        onClick={(e) => {
                          e.stopPropagation();
                          setConnectorsMenuOpen((v) => !v);
                        }}
                      >
                        <Icon name="plug" size={15} className="shrink-0 text-muted" />
                        {t("composer.attach_connectors")}
                        <Icon name="chevronRight" size={12} className="ml-auto text-faint" />
                      </button>
                      {connectorsMenuOpen && (
                        <div
                          className="absolute z-40 left-full top-0 ml-1 w-[240px] rounded-xl border border-line bg-panel shadow-2xl py-1.5"
                          onClick={(e) => e.stopPropagation()}
                          data-testid="attach-connectors-glance"
                        >
                          {connectorsGlance === null ? (
                            <div className="px-3 py-1.5 text-[12px] text-faint">
                              {t("composer.connectors_glance_loading")}
                            </div>
                          ) : (
                            <>
                              {connectorsGlance.connectors.filter((c) => c.connected).length === 0 &&
                              connectorsGlance.mcp.length === 0 ? (
                                <div className="px-3 py-1.5 text-[12px] text-faint">
                                  {t("composer.connectors_glance_empty")}
                                </div>
                              ) : (
                                <div className="px-3 py-1 flex flex-col gap-0.5">
                                  {connectorsGlance.connectors
                                    .filter((c) => c.connected)
                                    .map((c) => (
                                      <div key={c.name} className="text-[12.5px] text-ink truncate">
                                        {c.title}
                                      </div>
                                    ))}
                                  {connectorsGlance.mcp.length > 0 && (
                                    <div className="text-[12.5px] text-muted">
                                      {t("composer.connectors_glance_mcp_count", {
                                        count: connectorsGlance.mcp.length,
                                      })}
                                    </div>
                                  )}
                                </div>
                          ...
                          <button
                            className="w-full text-left px-3 py-1.5 rounded-lg hover:bg-paper text-[12px] text-accent"
                            data-testid="attach-connectors-manage"
                            onClick={() => {
                              setAttachMenuOpen(false);
                              setConnectorsMenuOpen(false);
                              props.onOpenConnectors?.();
                          ...
                          >
                            {t("composer.attach_connectors_manage")}
                          </button>
                        </div>
                      )}
                    </div>
                  ...
                  {/* Skills (U2): a plain shortcut, same shared library as any other agent — no
                      new backend concept, so no picker/glance needed here. */}
                  {props.onOpenSkills && (
                    <button
                      className="w-full flex items-center gap-2.5 px-3 py-1.5 text-[13px] text-left hover:bg-paper"
                      data-testid="attach-skills-row"
                      onClick={() => {
                        setAttachMenuOpen(false);
                        props.onOpenSkills?.();
                      }}
          ...
          {!dictation?.recording && (needsModel ? (
            ...
            <>
              ...
              {/* Model management: opens Settings ▸ Models directly — the existing
                  add/remove-model + worker-pool-checkbox surface (ModelsTab /
                  ModelChecklist), not a separate page. */}
              {props.onOpenModels && (
                <button
                  className={iconBtn}
                  title={t("model_manage.open")}
                  aria-label={t("model_manage.open")}
                  data-testid="model-manage-open"
...
function attachItem(icon: "image", label: string, onClick: () => void) {
  return (
    <button
      className="w-full flex items-center gap-2.5 px-3 py-1.5 text-[13px] text-left hover:bg-paper"
      onClick={onClick}
