...
/** Native single-file picker (Team Setup U3), same dual-path pattern as chooseFolder. */
...
export async function chooseFile(): Promise<string | null> {
  if (isTauri()) return pickFile();
  const { pickFileViaServer } = await import("./api");
  return pickFileViaServer();
}
