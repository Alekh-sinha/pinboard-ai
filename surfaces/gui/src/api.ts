...
/** Single-file counterpart (Team Setup U3): the "+" menu's Files/Folder attach needs a real
 * path for a single file too — same sidecar-dialog rationale as pickFolderViaServer. */
export async function pickFileViaServer(): Promise<string | null> {
  try {
    const res = await fetch(`${httpBase()}/v1/workspaces/pick-file`, { method: "POST" });
    const d = await res.json();
    return d.ok && d.path ? d.path : null;
  } catch {
    return null;
...
export interface Persona {
  ...
  version?: string;
  installed_at?: string;
  // "lead" | "worker" | null — a persona with no `team:` trait is solo-only.
  team?: string | null;
}
...
// -- Team Setup: register a worker/subagent, per-team grants, RAG ingest ------
// registerSubagent wraps the exact same install-then-consent path as installPersona
// (a generated team:worker manifest, not a new install mechanism) — reuse
// ConsentCard for the result, same as PersonasTab does for a regular install.
export async function registerSubagent(body: {
  name: string;
  tagline: string;
  description: string;
  ...
  const res = await fetch(`${httpBase()}/v1/subagents/register`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify(body),
  });
...
export async function getWorkerModelPool(): Promise<string[]> {
  const res = await fetch(`${httpBase()}/v1/team-setup/worker-model-pool`);
  return (await res.json()).models ?? [];
}

export async function setWorkerModelPool(models: string[]): Promise<{ ok: boolean; models: string[] }> {
  const res = await fetch(`${httpBase()}/v1/team-setup/worker-model-pool`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
...
export interface TeamGrant {
  connectors: string[];
  mcp: string[];
  scope: "lead" | "worker" | "both";
}

export async function getTeamGrants(teamId: string): Promise<TeamGrant[]> {
  const res = await fetch(`${httpBase()}/v1/teams/${encodeURIComponent(teamId)}/grants`);
...
export async function addTeamGrant(
  teamId: string,
  grant: TeamGrant,
): Promise<{ ok: boolean; grants?: TeamGrant[]; error?: string }> {
  const res = await fetch(`${httpBase()}/v1/teams/${encodeURIComponent(teamId)}/grants`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
...
export async function clearTeamGrants(teamId: string): Promise<{ ok: boolean }> {
  const res = await fetch(`${httpBase()}/v1/teams/${encodeURIComponent(teamId)}/grants`, {
    method: "DELETE",
  });
  return res.json();
...
/** Deterministic (non-LLM-triggered) ingestion: the UI calls this directly on a
 * folder/file attach, never via a model tool call — see coworker/server/manager.py's
 * start_rag_ingest for why that matters. Poll getRagIngestStatus(job_id) for progress. */
export async function startRagIngest(
  ...
  const res = await fetch(`${httpBase()}/v1/rag/ingest`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ path, collection, workspace, ocr }),
  });
  return res.json();
}

export async function getRagIngestStatus(jobId: string): Promise<RagIngestStatus> {
  const res = await fetch(`${httpBase()}/v1/rag/ingest/${encodeURIComponent(jobId)}`);
  return res.json();
}
...
export interface SkillRow {
  ...
  description: string;
  instructions: string;
  scope: "global" | "project" | "lead" | "worker";
...
// Team page (general-lead/general-worker skill panels): skills scoped to exactly one
// of those two personas, never merged with global/project — see SkillStore's
// rows_for_scope / GET /v1/skills/persona/{scope}. Create/update/delete/move reuse
// createSkill/updateSkill/deleteSkill/moveSkill unchanged (scope: "lead"|"worker").
export async function getPersonaSkills(scope: "lead" | "worker"): Promise<SkillRow[]> {
  const res = await fetch(`${httpBase()}/v1/skills/persona/${scope}`);
  return (await res.json()).skills ?? [];
}
