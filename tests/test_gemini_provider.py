...
def test_complete_parses_function_calls_with_synthesized_ids():
    ...
    # Gemini gives no call id natively, so we synthesize one — it must be genuinely
    # unique (not derived from the in-turn position, which collided across a whole
    # session's worth of turns — see gemini_provider.py's comment at the call site)
    # and every call still gets its own id.
    assert [c.name for c in turn.tool_calls] == ["write_file", "read_file"]
    ids = [c.id for c in turn.tool_calls]
    assert len(set(ids)) == len(ids)
    assert all(cid.startswith("call_") for cid in ids)
    assert turn.tool_calls[0].arguments == {"path": "a.txt"}


def test_synthesized_ids_dont_collide_across_separate_turns():
    """Regression test: a real session hit this — every turn's single tool call
    got the same id ("call_0", derived from the in-turn list position), which fed
    the Inbox's idempotent-by-tool_call_id approval guard and made a much later,
    unrelated write_file call silently inherit an earlier turn's already-resolved
    (denied) decision instead of ever prompting the user again. Each turn here
    only has ONE call each — exactly the shape that triggered it, since a
    same-index collision needs no more than that."""
    provider = GeminiProvider(
        client=_FakeClient(response=_response([_call_part("write_file", {"path": "a"})]))
    )
    first = provider.complete(model="m", messages=[{"role": "user", "content": "go"}])
    provider2 = GeminiProvider(
        client=_FakeClient(response=_response([_call_part("write_file", {"path": "a"})]))
    )
    second = provider2.complete(model="m", messages=[{"role": "user", "content": "again"}])
...
def test_stream_collects_function_calls_across_chunks():
    ...
    # Same uniqueness guarantee as the non-streaming path — see the comment above.
    assert [c.name for c in final.tool_calls] == ["f", "g"]
