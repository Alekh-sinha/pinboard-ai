...
- **Sticky-note redesign of the Automations page** (`surfaces/gui/src/components/ScheduledView.tsx`, `AutomationQuickstart.tsx`, `surfaces/gui/src/noteVariant.ts`) — the task board, the "New automation" form, the template picker, and a task's detail view all render as sticky notes on a corkboard (deterministic per-task color/tilt, a responsive auto-fill grid so it holds up from a handful of automations to dozens), reusing the visual language already shipped in `SlackHowItWorks.tsx`'s post-it annotations rather than inventing a new one.
