...
# Persona-scoped skills (Team page, general-lead/general-worker): a skill added "for
# lead" and one added "for worker" must never cross — each gets its own dedicated,
# unfiltered directory under state_dir(), same posture as the global scope (no
# manifest allowlist involved). These two personas are builtin manifests whose own
# folder lives in the installed app, not user data (see _base()'s comment below), so
# these dirs can't just be "next to the manifest" the way a bundled worker's skills/
# folder is.
LEAD_SCOPE = "lead"
WORKER_SCOPE = "worker"
...
class SkillStore:
    ...
    def _base(self, scope: str, workspace: Optional[str | Path]) -> Path:
        ...
        if scope == LEAD_SCOPE:
            # general-lead is a builtin manifest under coworker/personas/builtin/ —
            # part of the installed app, not user data — so its skills live under
            # state_dir() (the same writable-user-state area global_dir already
            # uses) rather than beside the manifest.
            return self.global_dir.parent / "persona-skills" / "general-lead"
        if scope == WORKER_SCOPE:
            return self.global_dir.parent / "persona-skills" / "general-worker"
        raise ValueError(f"Unknown scope: {scope}")

    def dir_for_scope(self, scope: str, workspace: Optional[str | Path] = None) -> Path:
        """Public accessor for `_base()` — for callers outside this class that need the
        actual directory (e.g. wiring a persona's skill dir into extra_skill_dirs)."""
        return self._base(scope, workspace)
    ...
    def find(
        ...
        for scope, base in ((LEAD_SCOPE, self._base(LEAD_SCOPE, None)), (WORKER_SCOPE, self._base(WORKER_SCOPE, None))):
            if (base / name / "SKILL.md").is_file():
                return self._folder_of(base, name), scope
        if (self.global_dir / name / "SKILL.md").is_file():
            return self._folder_of(self.global_dir, name), GLOBAL_SCOPE
        raise ValueError(f"Unknown skill: {name}")

    def _rows_in(self, base: Path, scope: str, disabled: set[str]) -> list[dict[str, Any]]:
    ...
    def rows(self, workspace: Optional[str | Path] = None) -> list[dict[str, Any]]:
        ...
        for base, scope in scopes:
            for row in self._rows_in(base, scope, disabled):
                if row["name"] in seen:  # project copy shadows the global one
                    out[seen[row["name"]]] = row
                else:
                    seen[row["name"]] = len(out)
                    out.append(row)
        return out

    def rows_for_scope(self, scope: str) -> list[dict[str, Any]]:
        """Skills in exactly one persona-scoped dir (LEAD_SCOPE/WORKER_SCOPE), no
        merging with global/project — used for the Team page's per-persona skill
        panel, where mixing scopes would misrepresent what that persona alone has."""
        return self._rows_in(self._base(scope, None), scope, self.disabled_names())
