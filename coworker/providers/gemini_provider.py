...
def _parse_candidate(response: Any) -> _Parsed:
    ...
    for part in getattr(content, "parts", None) or []:
        ...
        if function_call is not None:
            out.calls.append(
                ToolCall(
                    # Try a real id first (matches the pattern every other provider
                    # uses — Anthropic/Bedrock/OpenAI Responses all do `getattr(x,
                    # "id", "")`) — today's Gemini SDK objects don't carry one, but
                    # this stays forward-compatible if a future API version adds it,
                    # rather than silently discarding it. Falls back to "" (filled in
                    # by the caller) only when genuinely absent.
...
class GeminiProvider(ProviderClient):
    ...
    def complete(
        ...
        **settings: Any,
        ...
        # Today's Gemini SDK gives no call id (_parse_candidate's `id` comes back "");
        # invent one only then. A prior bug here derived it from this turn's local
        # list index, which collided across every turn of a session (every turn's
        # first call became "call_0") — that fed the Inbox's idempotent-by-
        # tool_call_id approval guard (inbox.py), so a later, unrelated tool call
        # would silently inherit an EARLIER turn's already-resolved (and possibly
        # denied) decision instead of ever prompting again. Must be unique across the
        # whole session, not just within one response.
        tool_calls = [
            ToolCall(id=c.id or f"call_{uuid.uuid4().hex[:12]}", name=c.name, arguments=c.arguments)
    ...
    def stream(
        ...
        **settings: Any,
        ...
        # Same fix as the non-streaming path above: use a real id if one's ever
        # present, else invent one that's unique across the whole session, not
        # derived from this turn's local call index.
        tool_calls = [
            ToolCall(id=c.id or f"call_{uuid.uuid4().hex[:12]}", name=c.name, arguments=c.arguments)
            for c in calls
        ]
