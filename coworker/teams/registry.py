...
class TeamWorker:
    ...
    persona: str
    session_id: str
    model: str = ""  # the CURRENTLY bound model — matches SessionRecord.model, moves on failover
    reason: str = ""  # why the lead staffed it — surfaces in teammates' rosters
    # Ordered fallback pool (models[0] is the primary, same value as `model` at staffing
    # time). On a quota/access-classified failure, deliver_to_session switches to the
    # next untried entry and retries the same turn rather than surfacing an error the
    # worker has no way to act on (see engine.py's retry() — "switching models and THEN
    # retrying is the intended recovery path"). Empty = no configured fallback; today's
    # plain stop-and-surface behavior applies unchanged.
...
class TeamRegistry:
    ...
    def set_worker_model(self, team_id: str, session_id: str, model: str) -> None:
        """Keep the roster's displayed model in sync after an automatic failover
        (deliver_to_session) — the session's own record is the source of truth for
        what actually runs; this just keeps board/roster views from showing stale
        info after a switch."""
        with self._lock:
