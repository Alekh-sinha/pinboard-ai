...
class PersonaRegistry:
    ...
    def worker_catalog(self) -> list[dict]:
        """Enabled `team: worker` personas (id, tagline, description) — for injecting into
        a lead's context so it can discover and staff workers, including ones the user
        just registered, without them being hardcoded into its own system prompt. Mirrors
        the visibility rule `_visible` already applies elsewhere (unshipped-but-not-
        internal personas stay hidden unless the user explicitly enabled one)."""
        out = []
        for e in self._entries.values():
            if e.manifest is None or e.manifest.team != "worker":
                continue
            if not (self._visible(e) and self.is_enabled(e.id)):
                continue
            out.append(
                {
                    "id": e.id,
        ...
        return [
            {
                ...
                # "lead" | "worker" | None — lets the Team Setup UI show the worker
                # catalog without a second endpoint.
                "team": e.manifest.team if e.manifest else None,
            }
            for e in self._entries.values()
            if self._visible(e)
        ]
