...
def build_engine(
    *,
    ...
    # A lead's worker-discovery menu (mirrors skill_filter's live-callable shape): called
    # per turn, not snapshotted at build, so a worker registered/enabled mid-session is
    # visible to the lead from its next message, no restart needed. Only ever consulted
    # for `agent.team == "lead"`.
    worker_catalog: Optional[Callable[[], list[dict[str, Any]]]] = None,
    # A lead's live model list — same shape/rationale as worker_catalog, so the model
    # a lead proposes for a staffed worker (propose_team's model/models fields) is
    # informed by what's actually configured in this installation, not a static
    # manifest guess. Backed by the same _curated_models() the composer's own picker
    # uses. Only ever consulted for `agent.team == "lead"`.
    model_catalog: Optional[Callable[[], list[str]]] = None,
) -> TurnEngine:
    ...
    # Browser tooling (headed connector tools + the reasoning below also covers the
    # headless browser-pool MCP scope, see persona_mcp_scope's caller) defaults ON for
    # TEAM-CAPABLE personas only (lead/worker — research-lead and future Team Setup
    # subagents) — not the whole catalog. Existing curated solo personas (code_agent,
    # myhelper_agent, etc.) deliberately ship without browser and keep that scoping
    # untouched; overturning it broke test_engine_connector_tools_are_cowork_scoped's
    # asserted design and was explicitly walked back after review. A team persona opts
    # out with `exclude_tools: [browser]`, same list already used to suppress web_search.
    connectors_decl = agent.connectors
    if (
        agent.team is not None
        and "browser" not in agent.exclude_tools
        and connectors_decl is not True
    ):
        connectors_decl = tuple(set(connectors_decl or ()) | {"browser"})
    if connectors_decl:
        ...
        # connectors it declared (now implicitly including "browser" unless excluded,
        # see above) — an undeclared connector's tools never enter the session, no
        # matter what the user has connected. True = general personas (Cowork) that
        # legitimately drive whatever is connected.
        if connectors_decl is not True:
            enabled_connectors = enabled_connectors & set(connectors_decl)
    ...
    # unless the persona brings its own search tools (MCP-backed) and opts out via
    # `exclude_tools` — avoids a redundant, confusable second search tool.
    if "web_search" not in agent.exclude_tools:
    ...
    def context_provider() -> str:
        ...
        # Worker-discovery menu, lead-only, recomputed every turn same as the skill
        # catalog above — a worker registered/enabled mid-session must be visible to
        # the lead without a restart.
        if agent.team == "lead" and worker_catalog is not None:
            from .teams.catalog import worker_catalog_text

            roster_ctx = worker_catalog_text(worker_catalog())
            if roster_ctx:
                parts.append(roster_ctx)
        # Live model list, lead-only, same per-turn-recompute posture as the worker
        # catalog above — a model added/removed mid-session applies from the next
        # message.
        if agent.team == "lead" and model_catalog is not None:
            from .teams.catalog import model_catalog_text

            models_ctx = model_catalog_text(model_catalog())
            if models_ctx:
                parts.append(models_ctx)
        return "\n\n".join(parts)
