...
TOOL_DEFS: tuple[ConnectorToolDef, ...] = (
    ...
    ConnectorToolDef(
        "browser",
        # Writes the site's session/cookie state to disk (browser_logins.py) — a local
        # write with the same footing as browser_screenshot above.
        "browser_save_login",
        "Stay logged in",
        "write",
        "Save the current browser session's login state for a site.",
    ),
    ConnectorToolDef(
        "browser",
        ...
        "Forget login",
        "write",
        "Delete a previously saved login for a site.",
    ),
    ConnectorToolDef(
        "github",
        "github_search",
