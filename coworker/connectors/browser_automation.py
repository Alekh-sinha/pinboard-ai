...
class _BrowserController:
    ...
    def save_login(self, site: str) -> dict[str, Any]:
        return self._submit(lambda: self._save_login_locked(site))

    def _save_login_locked(self, site: str) -> dict[str, Any]:
        from ..browser_logins import normalize_site, save as save_login_state

        key = normalize_site(site)
        if not key:
            return {"error": f"invalid site name: {site!r}"}
        with self._lock:
            if self._context is None:
                return {"error": "no browser session is open — nothing to save"}
            try:
                state = self._context.storage_state()
            except Exception as exc:
                return {"error": str(exc)}
        save_login_state(key, state)
        self._touch(last_action="save_login", last_result="ok", last_error="")
...
def browser_save_login_session(site: str) -> dict[str, Any]:
    return _BROWSER.save_login(site)


def browser_forget_login_session(site: str) -> dict[str, Any]:
    from ..browser_logins import forget, normalize_site

    key = normalize_site(site)
    if not key:
        return {"error": f"invalid site name: {site!r}"}
    return {"ok": forget(key)}
...
def make_browser_automation_tools(
    *, roots: Optional[list[Any]] = None
    ...
    def browser_save_login(site: str) -> dict[str, Any]:
        return browser_save_login_session(site)

    browser_save_login.__name__ = "browser_save_login"
    tools.append(
        _attach(
            browser_save_login,
            _schema(
                "browser_save_login",
                "Stay logged in on `site` (e.g. 'wsj', 'ft') by saving the CURRENT "
                "browser session's login state, so a future session can read this "
                "site without logging in again. This captures only the site's own "
                "session cookies/tokens — never a password, and never anything the "
                "model reads or types. Only works while the same session that just "
                "logged in is still open (browser_close must not have been called "
                "since). Always ask the user first and only call this if they say "
                "yes; if they decline, do not call it and nothing is written.",
                {"site": {"type": "string"}},
                ["site"],
            ),
            approval=True,
    ...
    def browser_forget_login(site: str) -> dict[str, Any]:
        return browser_forget_login_session(site)

    browser_forget_login.__name__ = "browser_forget_login"
    tools.append(
        _attach(
            browser_forget_login,
            _schema(
                "browser_forget_login",
                "Delete a previously saved login for `site` — the next visit will "
                "need a fresh manual login.",
                {"site": {"type": "string"}},
                ["site"],
            ),
            approval=True,
