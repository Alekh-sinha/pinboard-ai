...
from ..teams.grants import TeamGrant, TeamGrantStore
...
class SessionManager:
    def __init__(
        ...
        *,
        ...
        # Deterministic (non-LLM-triggered) RAG ingestion jobs — a UI "attach a folder"
        # action calls run_rag_ingest directly, bypassing the model's tool-calling loop
        # entirely, so ingestion never depends on an LLM remembering to call a tool.
        # In-memory only: a job is a progress-poll target for the life of one request,
        # not durable state that needs to survive a restart.
        self._rag_jobs: dict[str, dict[str, Any]] = {}
        ...
        self.team_grants = TeamGrantStore(base / "team_grants.json")
    ...
    def get_engine(
        ...
        *,
        ...
        # Per-team connector grant (Team Setup): additive only, on top of whatever the
        # persona's own manifest already declares — never a way for a persona to see
        # LESS than its manifest grants, only ever more, scoped to this one team.
        granted = self.granted_connectors_for_session(session_id)
        if granted and ag.connectors is not True:
            ag = dataclasses.replace(
                ag, connectors=tuple(set(ag.connectors or ()) | granted)
            )
        ...
        engine = build_engine(
            ...
            # Live, not a snapshot (same reason as skill_filter): a worker registered or
            # enabled mid-session must be discoverable by the lead from its next turn.
            worker_catalog=self.personas.worker_catalog,
            # Same live-callable posture, for the model list — a lead's propose_team
            # model choice draws on what's actually configured, not a static manifest
            # guess (see teams/catalog.py's model_catalog_text).
    ...
    async def prepare_mcp_tools(
        ...
        # Per-team MCP grant (Team Setup): additive only, same posture as the connector
        # grant above — only widens an already-scoped persona; a persona with no `mcp:`
        # list is already unscoped (every enabled server), so a grant here is a no-op.
        if persona_mcp is not None:
            granted_mcp = self.granted_mcp_for_session(session_id)
            if granted_mcp:
                persona_mcp = persona_mcp | granted_mcp
        persona_mcp_exclude = self.persona_mcp_exclude(agent)
        for server in load_mcp_servers(
            ...
            if not server.enabled:
                continue
            if server.name in persona_mcp_exclude:
                continue
    ...
    async def _find_mcp_server(self, name: str, workspace: Optional[str] = None):
        ws = workspace or self.default_workspace
        for server in load_mcp_servers(
            ws, secrets=self.secrets, workspace_trusted=self._mcp_workspace_trusted(ws)
        ):
            if server.name == name:
                return server
        return None

    _RAG_EXTS = (".pdf", ".xlsx", ".xlsm")
    ...
    async def start_rag_ingest(
        ...
        *,
        ...
        """The deterministic (non-LLM-triggered) ingestion entry point: a UI "attach a
        folder" action calls this directly — never a model tool call — so ingestion
        never depends on an LLM remembering to do it. Walks the path up front (cheap)
        to get a real total for progress, then launches the actual ingestion as a
        background task and returns immediately; poll rag_ingest_status(job_id).
        `ocr=False` is a hard, complete opt-out for every file in this job — passed
        straight through to the rag server's rag_ingest_path tool (see its docstring)."""
        root = Path(path).expanduser()
        if root.is_file():
            files = [root] if root.suffix.lower() in self._RAG_EXTS else []
        elif root.is_dir():
            files = sorted(
                f for f in root.rglob("*") if f.suffix.lower() in self._RAG_EXTS
            )
        else:
            return {"ok": False, "error": f"not a file or directory: {path}"}
        if not files:
            return {"ok": False, "error": f"no PDF/Excel files found at {path}"}

        job_id = uuid.uuid4().hex[:12]
        self._rag_jobs[job_id] = {
            "status": "running",
        ...
        asyncio.create_task(
            self._run_rag_ingest(job_id, files, collection, workspace, ocr)
        )
    ...
    async def _run_rag_ingest(
        ...
        job = self._rag_jobs[job_id]
        try:
            server = await self._find_mcp_server("rag", workspace)
            if server is None:
                job["status"] = "error"
                job["error"] = (
                    "no MCP server named 'rag' is registered — add it to mcp.json"
                )
                return
            conn = await self.mcp.ensure(server)
            for f in files:
                ...
                try:
                    result = await conn.session.call_tool(
                        "rag_ingest_path",
                        {"collection": collection, "path": str(f), "ocr": ocr},
                    )
                    text = result.content[0].text if result.content else "{}"
                    job["results"].append(json.loads(text))
                except Exception as exc:
                    job["results"].append({"filename": f.name, "error": str(exc)})
                job["done"] += 1
            job["current"] = None
            job["status"] = "done"
        except Exception as exc:
    ...
    def rag_ingest_status(self, job_id: str) -> dict[str, Any]:
        job = self._rag_jobs.get(job_id)
        if job is None:
    ...
    def create_team(
        ...
        for member in members:
            ...
            # A `models` list is an ordered fallback pool (primary first); a plain
            # `model` string is the single-model shorthand, unchanged from before.
            # The session binds to the primary; the rest are only consulted on a
            # quota/access-classified failure (see deliver_to_session).
            raw_models = member.get("models")
            pool = (
                [str(m).strip() for m in raw_models if str(m).strip()]
                if isinstance(raw_models, list)
                else []
            )
            # The lead's explicit choice always wins; Team Setup's default pool only
            # fills in when it specified neither `models` nor `model`.
            if not pool and not member.get("model"):
                pool = self.get_default_worker_model_pool()
            model = pool[0] if pool else str(member.get("model") or record.model)
    ...
    # -- web search -------------------------------------------------------------
    def get_default_worker_model_pool(self) -> list[str]:
        """User-level default fallback pool, applied when a lead's propose_team call
        doesn't specify one for a member (Team Setup's model-pool preference). A lead
        that DOES specify `models`/`model` always wins — this is only the fallback."""
        profile = self.secrets.get("team_setup:worker_model_pool") or {}
        return [str(m) for m in (profile.get("models") or []) if str(m).strip()]

    def set_default_worker_model_pool(self, models: list[str]) -> dict[str, Any]:
        pool = [str(m).strip() for m in models if str(m).strip()]
        self.secrets.put("team_setup:worker_model_pool", {"models": pool})
        return {"ok": True, "models": pool}
    ...
    def pick_native_file(self) -> dict[str, Any]:
        """Single-file counterpart to pick_native_folder (Team Setup U3): the "+" menu's
        Files/Folder attach routes a single file through RAG ingestion (start_rag_ingest
        already accepts a file path, not just a directory), which needs a real filesystem
        path a web file input can't give us."""
        import subprocess
        import sys
        ...
        if sys.platform == "darwin":
            cmd = [
                ...
                'tell application "System Events" to activate',
                "-e",
                'POSIX path of (choose file with prompt "Choose a file to add to the knowledge base")',
            ]
        elif sys.platform == "win32":
            ps = (
                "Add-Type -AssemblyName System.Windows.Forms; "
    ...
    async def deliver_to_session(
        ...
        if not self.try_mark_running(session_id):
            ...
            return
        try:
            # Model-pool failover (worker sessions only): a quota/access-classified
            # failure (engine.py's friendly_model_error — the ERROR payload carries
            # `raw` exactly when it classified one) switches to the next untried pool
            # entry and retries the SAME failed turn, rather than surfacing an error
            # the worker has no way to act on. `engine.retry()` is the existing,
            # designed primitive for this — its own docstring: "switching models and
            # THEN retrying is the intended recovery path." Pool exhausted (or no pool
            # configured, or a non-classified failure) falls through to today's
            # unchanged stop-and-record behavior.
            pool = self.model_pool_for_session(session_id)
            tried = {engine.model}
            events = engine.run(message, source=source)
            while True:
                ...
                async for event in events:
                    ...
                    if event.type.value == "error":
                        classified_error = event.data if "raw" in (event.data or {}) else None
                ...
                if classified_error is None or next_model is None:
                    if classified_error is not None:
                        # Classified, but the pool is exhausted — record it same as any
                        # other unrecoverable failure, now that switching won't help.
                        reason = classified_error.get("error", "unknown error")
                        logger.warning(
                            "background turn failed for %s after exhausting model "
                            "pool %s: %s", session_id, pool, reason,
                        )
                        self.unrouted.record(session_id, "-", message, reason=reason)
                    break
                tried.add(next_model)
                notice = engine.switch_model(next_model)  # appends its own history notice
                if notice:
                    await self.broadcast_session(
                        session_id,
                        {
                            "type": "model_changed",
                            "data": {"model": next_model, "text": notice},
                        },
                    )
                team = self.teams.for_worker_session(session_id)
                if team is not None:
                    self.teams.set_worker_model(team[0].team_id, session_id, next_model)
                events = engine.retry()
            self.save(session_id, engine)
        except (
            Exception
    ...
    def list_persona_skills(self, scope: str) -> list[dict[str, Any]]:
        """Skills scoped to general-lead or general-worker only (Team page) — see
        SkillStore.rows_for_scope. Never merges with global/project."""
        from ..skills.store import LEAD_SCOPE, WORKER_SCOPE

        if scope not in (LEAD_SCOPE, WORKER_SCOPE):
            raise ValueError(f"unknown persona skill scope: {scope}")
        return self.skill_store.rows_for_scope(scope)
    ...
    def model_pool_for_session(self, session_id: str) -> list[str]:
        """This session's ordered fallback pool (models[0] == the current model), or
        [] if it isn't a worker with one configured. Only workers get a pool — the
        lead's model is a plain single choice, same as any solo session."""
        found = self.teams.for_worker_session(session_id)
        if found is None:
            return []
        return list(found[1].models)

    def team_role_for_session(self, session_id: str) -> Optional[tuple[str, str]]:
        """(team_id, role) for a session that's a lead or worker on a team, else None.
        role is "lead" or "worker" — the vocabulary team_grants.py's TeamGrant.scope
        uses ("both" matches either)."""
        team = self.teams.for_lead_session(session_id)
        if team is not None:
            return team.team_id, "lead"
        found = self.teams.for_worker_session(session_id)
        if found is not None:
            return found[0].team_id, "worker"
        return None

    def granted_connectors_for_session(self, session_id: str) -> set[str]:
        """Extra connector ids this session's team has granted its role, on top of
        whatever the persona's own manifest already declares. Empty for a session
        that isn't on a team, or a team with no grants set up."""
        found = self.team_role_for_session(session_id)
        if found is None:
            return set()
        team_id, role = found
        return self.team_grants.connectors_for(team_id, role)

    def granted_mcp_for_session(self, session_id: str) -> set[str]:
        found = self.team_role_for_session(session_id)
        if found is None:
            return set()
        team_id, role = found
        return self.team_grants.mcp_for(team_id, role)
    ...
    def persona_mcp_exclude(self, persona_id: str) -> set[str]:
        """MCP server names this persona opts out of via `exclude_tools`, even when it
        is otherwise unscoped (persona_mcp_scope returns None = every enabled server).
        Reuses the same field that suppresses always-on built-ins like web_search
        (agent.py) — e.g. `exclude_tools: [browser-pool]` opts a persona out of the
        default-on headless browser without needing an `mcp:` allowlist at all."""
        entry = self.personas.get(persona_id)
        names = list((entry.manifest.exclude_tools if entry and entry.manifest else []) or [])
