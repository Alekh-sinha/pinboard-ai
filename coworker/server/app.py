...
def create_app(manager: SessionManager) -> FastAPI:
    ...
    @app.post("/v1/subagents/register")
    def register_subagent(body: dict) -> dict[str, Any]:
        """Team Setup's "register a worker" wizard: builds a team:worker manifest (+
        optional skill) from the wizard's fields and installs it through the exact same
        path as any other third-party persona — lands disabled pending the user's
        consent, same as POST /v1/personas/install (then enable via POST
        /v1/personas/{id} {enabled:true})."""
        from ..personas.manifest import ManifestError
        from ..personas.subagent_builder import build_worker_bundle, discard_bundle
        ...
        try:
            bundle = build_worker_bundle(
                ...
                persona_id=(str(body["persona_id"]) if body.get("persona_id") else None),
                connectors=list(body.get("connectors") or []) or None,
                mcp=list(body.get("mcp") or []) or None,
                scheduling=bool(body.get("scheduling", False)),
                skill_name=(str(body["skill_name"]) if body.get("skill_name") else None),
                skill_content=(
                    str(body["skill_content"]) if body.get("skill_content") else None
                ),
                tools=list(body.get("tools") or []) or None,
            )
        except ManifestError as e:
            return {"ok": False, "error": str(e)}
        try:
            summaries = manager.personas.install_from_dir(bundle)
        except Exception as e:
            return {"ok": False, "error": str(e)}
        finally:
            discard_bundle(bundle)
        return {"ok": True, "consent": summaries, "personas": manager.personas.list_all()}
    ...
    # Team page (general-lead / general-worker skill panels): skills scoped to exactly
    # one of those two personas, never merged with global/project — see
    # SkillStore.rows_for_scope. Create/update/delete/move reuse the existing routes
    # below unchanged (scope: "lead"|"worker" instead of "global"|"project").
    @app.get("/v1/skills/persona/{scope}")
    def persona_skills(scope: str) -> dict[str, Any]:
        try:
            return {"skills": manager.list_persona_skills(scope)}
    ...
    @app.post("/v1/workspaces/pick-file")
    async def pick_workspace_file() -> dict[str, Any]:
        # Single-file counterpart (Team Setup U3): the "+" menu's Files/Folder attach needs
        # a real path for a single file too, same rationale as the folder picker above.
        return await asyncio.to_thread(manager.pick_native_file)
    ...
    @app.get("/v1/teams/{team_id}/grants")
    def team_grants_list(team_id: str) -> dict[str, Any]:
        # Team Setup's connector/MCP tag control reads/writes these — additive grants
        # on top of whatever the lead/worker personas' own manifests already declare,
        # scoped to this one team (see teams/grants.py).
        from dataclasses import asdict

        return {"grants": [asdict(g) for g in manager.team_grants.get(team_id)]}

    @app.post("/v1/teams/{team_id}/grants")
    def team_grants_add(team_id: str, body: dict) -> dict[str, Any]:
        from ..teams.grants import TeamGrant, VALID_SCOPES

        scope = str(body.get("scope") or "both")
        if scope not in VALID_SCOPES:
            return {"ok": False, "error": f"scope must be one of {VALID_SCOPES}"}
        grant = TeamGrant(
            connectors=list(body.get("connectors") or []),
            mcp=list(body.get("mcp") or []),
            scope=scope,
        )
        manager.team_grants.add(team_id, grant)
        return {"ok": True, "grants": [g.__dict__ for g in manager.team_grants.get(team_id)]}

    @app.delete("/v1/teams/{team_id}/grants")
    def team_grants_clear(team_id: str) -> dict[str, Any]:
        manager.team_grants.clear(team_id)
        return {"ok": True}
    ...
    async def rag_ingest_start(body: dict) -> dict[str, Any]:
        # Deterministic ingestion trigger: this is a plain UI action, not a model tool
        # call, on purpose — ingestion must never depend on an LLM remembering to do
        # it. Starts a background job (a folder can take real time) and returns a
        # job_id to poll at GET /v1/rag/ingest/{job_id}. Async route (not sync def):
        # the background task it launches needs the request's own running event loop,
        # which a sync route's threadpool execution does not have.
        path = str(body.get("path") or "")
        collection = str(body.get("collection") or "")
        if not path or not collection:
            return {"ok": False, "error": "path and collection are required"}
        return await manager.start_rag_ingest(
            path,
            collection,
            workspace=body.get("workspace"),
            ocr=bool(body.get("ocr", True)),
        )

    @app.get("/v1/rag/ingest/{job_id}")
    def rag_ingest_progress(job_id: str) -> dict[str, Any]:
        return manager.rag_ingest_status(job_id)
    ...
    @app.get("/v1/team-setup/worker-model-pool")
    def worker_model_pool_get() -> dict[str, Any]:
        return {"models": manager.get_default_worker_model_pool()}

    @app.post("/v1/team-setup/worker-model-pool")
    def worker_model_pool_set(body: dict) -> dict[str, Any]:
        return manager.set_default_worker_model_pool(list((body or {}).get("models") or []))
